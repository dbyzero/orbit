...
export function loadLevelGraphic(levelJson) {
    const state = store.getState();
    const { graphicEngine, debugLayer } = state.gameEngine;

    // load background
    const bgLevel = PIXI.Sprite.from('level.png');
    bgLevel.x = 0;
    bgLevel.y = 0;
    bgLevel.width = levelJson.width;
    bgLevel.height = levelJson.height;
    bgLevel.zIndex = 0;
    graphicEngine.stage.addChild(bgLevel);

    // Draw debug for collision
...
export function updateGraphicEngine() {
    ...
    state.gameScene.physicalItems.forEach(physicalItem => {
        physicalItem.update();
    });
    ...
    // camera follow player if in normal mode
...
export function showTest() {
    const state = store.getState();
    const { graphicEngine } = state.gameEngine;
    ...
    spriteAnimated.y = 175;
    spriteAnimated.width = 50;
